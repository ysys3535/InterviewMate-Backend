...
public class GeminiService {
    ...
    public QuestionGenerateResponse generateQuestion(QuestionGenerateRequest request) {
        ...
        String prompt = """
                ...
                - COMMON 모드는 신입/주니어도 답변할 수 있는 적당한 난이도로 묻는다.
                - ADVANCED 모드는 답변의 근거, 한계, 실무 판단을 더 깊게 검증한다.

                질문 생성 규칙:
                1. 질문은 반드시 하나만 작성한다.
                2. 설명, 번호, 따옴표, 마크다운 없이 질문 문장만 반환한다.
                3. 질문은 한국어로 작성한다.
                4. 질문 순서가 첫 번째와 마지막이 아닌 경우, 이전 답변에서 나온 주장, 경험, 기술 선택, 성과, 실패, 의사결정 근거를 파고드는 후속질문으로 생성한다.
                ...
                9. COMMON 모드에서는 과도한 압박, 복잡한 시스템 설계, 깊은 내부 구현, 장애 원인 분석처럼 어려운 질문을 피한다.
                10. COMMON 모드에서는 경험 설명, 선택 이유, 협업 방식, 기본 문제 해결 방식처럼 답변하기 쉬운 질문을 만든다.
                11. ADVANCED 모드에서만 기술적 깊이, 트레이드오프, 장애 대응, 성능/보안/확장성 판단을 강하게 묻는다.
                ...
                모드별 질문 구성:
                - COMMON: 총 4개 질문으로 진행한다. 1번째는 자기소개, 2~3번째는 쉬운 후속질문 2개, 4번째는 기타 인터뷰로 하고 싶은 말을 묻는다.
                  COMMON 후속질문은 사용자가 답변한 경험을 조금 더 구체화하는 정도로 묻고, 면접 초중반 수준의 자연스러운 질문으로 만든다.
